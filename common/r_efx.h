...
#define FTENT_SPARKSHOWER       (1 << 17)
#define FTENT_NOMODEL           (1 << 18) // Doesn't have a model, never try to draw (it just triggers other things)
#define FTENT_CLIENTCUSTOM      (1 << 19) // Must specify callback. Callback function is responsible for killing tempent and updating fields (unless other flags specify how to do things)
#define FTENT_CUSTOM_SPR_RENDER (1 << 20) // Use the custom sprite renderer
